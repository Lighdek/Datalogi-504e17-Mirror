...
model = Sequential([
    ...
    Conv2D(filters=8, kernel_size=5, activation='relu', padding='same', input_shape=(256, 256, 3),
           kernel_initializer='random_uniform',
           bias_initializer='zeros'
           ),
    MaxPool2D(padding='same'),  # 64
    Conv2D(filters=8, kernel_size=5, activation='relu', padding='same',
           kernel_initializer='random_uniform',
           bias_initializer='zeros'
           ),
...
if __name__ == '__main__':
    ...
    while True:
        ...
        model.fit(np.array(images), labels, batch_size=18, epochs=50, verbose=1, validation_split=0.25)
        model.summary()
        ...
        for k in range(mw.shape[3]):
            thek = (normalized(mw[:,:,:,k]+1, axis=2))*255 #TODO: testing norm axis=2
            print(thek)
